...
export default function ManagerPermissionDetailPage() {
  ...
  const fetchPermissionDetails = async () => {
    ...
    try {
      ...
      response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/manager/permission/department/${permissionId}/`, {
        headers: {
      ...
      if (!response.ok) {
        ...
        response = await fetch(
          `${process.env.NEXT_PUBLIC_API_URL}/api/manager/permission/association/${permissionId}/`,
          {
            headers: {
              Authorization: `Bearer ${localStorage.getItem("accessToken")}`,
            },
      ...
      if (!response.ok) {
        ...
        response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/manager/permission/service/${permissionId}/`, {
          headers: {
            Authorization: `Bearer ${localStorage.getItem("accessToken")}`,
          },
        })
  ...
  const handleToggleStatus = async () => {
    try {
      ...
      if (permission.Department) {
        response = await directApi.manager.updateDepartmentPermission(permissionId, { Active: newStatus })
      } else if (permission.Association) {
        response = await directApi.manager.updateAssociationPermission(permissionId, { Active: newStatus })
      } else if (permission.PublicService) {
        response = await directApi.manager.updateServicePermission(permissionId, { Active: newStatus })
      }
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      if (permission.Department) {
        response = await directApi.manager.deleteDepartmentPermission(permissionId)
      } else if (permission.Association) {
        response = await directApi.manager.deleteAssociationPermission(permissionId)
      } else if (permission.PublicService) {
        response = await directApi.manager.deleteServicePermission(permissionId)
      }
      ...
      if (response && response.ok) {
        ...
        router.push("/manager/permissions")
      } else {
        throw new Error(`Failed to delete permission: ${response?.statusText || "Unknown error"}`)
      }
    } catch (error) {
          ...
          {permission && (
            <div className="flex gap-2">
              <Button asChild variant="outline">
                <Link href={`/manager/permissions/${permissionId}/edit`}>
        ...
        {isLoading ? (
          ...
          <Card>
            ...
            <CardFooter>
              <div className="flex gap-2">
                <Button asChild variant="outline">
                  <Link href="/manager/permissions">Back to Permissions</Link>
                </Button>
              </div>
            </CardFooter>
          </Card>
