...
export default function RequestDetailPage() {
  ...
  useEffect(() => {
    if (!authLoading && (!user || user.role !== "citizen")) {
      toast({
        title: "Access denied",
        description:
          "You must be logged in as a citizen to view request details.",
        variant: "destructive",
      });
  ...
  const handleAccessService = async () => {
    ...
    try {
      ...
      const serviceUrl = `${gatewayUrl}/${request.PublicService.MachineName}/`;
      ...
      if (response.status === 401) {
        toast({
          title: "Access denied",
          description:
            "You don't have permission to access this service. Please request access first.",
          variant: "destructive",
        });
        return;
      } else if (!response.ok) {
                  ...
                  {request.Granted === true && (
                    <div className="bg-green-50 dark:bg-green-900/20 p-4 rounded-lg border border-green-100 dark:border-green-900/50">
                      ...
                      <p className="text-sm text-green-700 dark:text-green-400">
                        Your request has been approved and a grant has been
                        issued.
                      </p>
                ...
                <CardContent className="space-y-3">
                  <div>
                    ...
                    <p className="font-medium">
                      {request.PublicService?.Title || "N/A"}
                    </p>
                  </div>
                  <div>
                    <h3 className="text-sm font-medium text-slate-500 dark:text-slate-400 mb-1">
                  ...
                  <div>
                    ...
                    <p className="text-sm">
                      {request.Citizen?.FirstName} {request.Citizen?.SecondName}{" "}
