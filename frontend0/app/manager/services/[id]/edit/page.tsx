...
export default function EditServicePage() {
  ...
  const fetchServiceDetails = async () => {
    ...
    setIsLoading(true)
    try {
      const response = await directApi.manager.getService(serviceId)
      if (!response.ok) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await directApi.manager.updateService(serviceId, formData)
      if (!response.ok) {
      ...
      <div className="space-y-6">
        <div className="flex items-center gap-2">
          <Button variant="ghost" size="icon" asChild>
            <Link href={`/manager/services/${serviceId}`}>
            ...
            <form onSubmit={handleSubmit}>
              ...
              <CardFooter className="flex justify-between">
                <Button type="button" variant="outline" asChild>
                  <Link href={`/manager/services/${serviceId}`}>Cancel</Link>
                </Button>
